...
class Solver_makeupGAN(object):
    def __init__(self, data_loaders, config, dataset_config):
        ...
        writer_path = os.path.join('/kaggle/working/', 'runs', config.task_name)
        print('TensorBoard will be saved in: ', writer_path)
        self.writer = SummaryWriter(writer_path)
        if not os.path.isdir(os.path.join('/kaggle/working/', 'runs', config.task_name)):
            os.makedirs(os.path.join('/kaggle/working/runs', config.task_name))
        #for recording
    ...
    def criterionHis(self, input_data, target_data, mask_src, mask_tar, index):
        ...
        mask_src = mask_src.expand(self.batch_size, 3, mask_src.size(2), mask_src.size(2)).squeeze()
        mask_tar = mask_tar.expand(self.batch_size, 3, mask_tar.size(2), mask_tar.size(2)).squeeze()
    ...
    def test(self):
        ...
        time_total = 0
        for i, (img_A, img_B) in enumerate(tqdm(self.data_loader_test)):
            #start = time.time()
            start = time.time()
            real_org = self.to_var(img_A)
            real_ref = self.to_var(img_B)
